...
# from PMPredictionFramework.PreProcessing.constants import project_mapping, calibration_columns_pm, calibration_columns_ox, calibration_columns_no2
# from PMPredictionFramework.PreProcessing.decrypt_file import *
# from PMPredictionFramework.PreProcessing.misc_utils import get_datastore_client, get_from_entity_if_present, get_project_for_subject, filter_out_outliers_gas

from PreProcessing.constants import project_mapping, calibration_columns_pm, calibration_columns_ox, calibration_columns_no2
from PreProcessing.decrypt_file import *
from PreProcessing.misc_utils import get_datastore_client, get_from_entity_if_present, get_project_for_subject, filter_out_outliers_gas
...
def load_personal_airspeck_file(subject_id, project_name=None, upload_type='automatic', is_minute_averaged=True,
                                ...
                                filter_pm=True, country_name=None, folder_name=None):
    '''
    ...
    if folder_name is None:
        folder_name = project_mapping[project_name][2]
    else:
        folder_name = '/' + folder_name + '/'

    print("Loading file: {}".format(folder_name + filename))
    data = load_airrespeck_file(folder_name + filename,
                                project_name)
    
    if filter_pm and data is not None and len(data) > 0:
        below_zero_mask = data['pm2_5'] <= 0
        very_low_mask = [bin0 <= 1 or bin1 <= 1 for bin0, bin1 in zip(data['bin0'], data['bin1'])]

        if np.count_nonzero(below_zero_mask) or np.count_nonzero(very_low_mask) :
            if not suppress_output:
                print("Setting {} values equal to or below 0, and {} very low values to {}".format(
                    np.count_nonzero(below_zero_mask), np.count_nonzero(very_low_mask), set_below_zero_to))
            data.loc[below_zero_mask | very_low_mask, 'pm2_5'] = set_below_zero_to
...
def load_respeck_file(subject_id, project_name=None, filter_out_not_worn=True, subject_visit_number=None,
                      upload_type='automatic', suffix_filename="", raw_file=False, directory=None):
    '''
    ...
    if directory is None:
        filepath = project_mapping[project_name][2] + filename
    else:
        filepath = directory + filename
       

    print("Loading file: {}".format(filepath))
    respeck_data = load_airrespeck_file(filepath, project_name)
...
def load_static_airspeck_file(sid_or_uuid, project_name=None, sensor_label=None, suffix_filename="",
                              ...
                              country_name=None, folder_name=None, filter_pm=True, set_below_zero_to=np.nan,
                              suppress_output=False):
    assert upload_type in ['automatic', 'sd_card'], "upload_type has to be either 'automatic' or 'sd_card', not '{}'".format(upload_type)
    ...
    if folder_name is None:
        ...
        folder_name = '/' + folder_name + '/'
    print("Loading file: {}".format( folder_name + filename))
    data = load_airrespeck_file(folder_name + filename, project_name)
    ...
    if filter_pm and data is not None and len(data) > 0:
        ...
        # Fix humidity values. Sometimes valid readings of humidity pass 100. Above 105, they are definitely invalid
        data.loc[data['humidity'] > 105, 'humidity'] = np.nan
        data.loc[data['pm2_5'] > 2000, 'pm2_5'] = np.nan
...
def load_merged_personal_data(subject_id, project_name=None, upload_type='automatic', subject_visit_number=None,
                              calibrate_pm=False, use_all_features_for_pm_calibration=False, airspeck_folder=None):
    ...
    if airspeck is None or respeck is None:
        ...
        if 'pm2_5' in respeck.columns:
            respeck = respeck.drop(columns=['pm2_5'])
        if 'timestamp.1' in airspeck.columns:
            airspeck = airspeck.drop(columns=['timestamp.1'])
...
def set_breathing_rate_nan_when_not_worn(respeck_data):
    ...
    not_worn_pred = (((resampled_data_mean['activity_level'] < 0.013) & (resampled_data_mean['breathing_rate'] > 24.5)) | (resampled_data_mean['activity_level'] < 0.008))
...
def get_uuid_for_subj_id_airspeck_personal(subject_id, timestamp=None, timeframe=None):
    ...
    if timeframe is not None:
        filters.extend([('timestamp', '>=', timeframe[0]),
...
def calibrate_airspeck(subj_id_or_uuid, airspeck_data, project_name, calibrate_pm=False, calibrate_no2=False, calibrate_ox=False,
    ...
    if airspeck_data is not None and len(airspeck_data) > 0:
        ...
        if calibrate_no2 or calibrate_ox:
            airspeck_data = filter_out_outliers_gas(airspeck_data)
...
def get_calibration_factors_airspeck(subj_or_uuid, to_be_calibrated_timestamp=None, project_name=None, calibrate_pm=False,
    ...
    if len(results_pm) > 0 and not (len(results_pm) == 1 and results_pm[0]['calibration_id'] == '2019-11 EBAM IITD'):
        ...
        time_difference = timedelta(days=2000) #Approx 6 years as initial time difference
        print(time_difference)
        print(type(time_difference))
            
        if len(results_pm) > 0:
            for idx, result in enumerate(results_pm[0:]):
                if result['calibration_id'] == '2019-11 EBAM IITD':
                    continue
                if result['calibration_id'] == 'CPCB Oct17-Jan18':
                    continue
                ...
                if abs(new_time_difference.days) < abs(time_difference.days):
                    ...
                    result_to_use_idx = idx
                    #BUT if we were comparing against november, we actually want the bigger one
    ...
    if len(results_no2) > 0:
        ...
        if len(results_no2) == 1:
            ...
            #result_date_no2 = result_no2['time_of_calibration'].astimezone(tz).date()
...
def calibrate_with_factors(data, factors):
    ...
    ## Calibrate NO2
    was_calibrated_no2 = False
    if len(low_humidity_factors_no2) > 0 and (len(low_humidity_factors_no2) == len(calibration_columns_no2) + 1):
    ...
    ##Calibrate OX
    was_calibrated_ox = False
    if len(low_humidity_factors_ox) > 0 and (len(low_humidity_factors_ox) == len(calibration_columns_ox) + 1):
